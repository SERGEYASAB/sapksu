...
ErrorCodes::ErrorCodes(int comProtocolCode, QWidget *parent) : QWidget(parent)
{
    ...
    m_view = new CustomizableTableView("ErrorCodes");
    ...
    connect(m_view, &QTableView::doubleClicked, this, &ErrorCodes::edit);
    ...
    connect(m_addButon, &PushButtonPlus::clicked, this, &ErrorCodes::add);
    connect(m_removeButton, &PushButtonPlus::clicked, this, &ErrorCodes::remove);
...
void ErrorCodes::edit()
{
    if (!m_view->currentIndex().isValid()) {
        return;
    }

    ErrorCodesDialogMapper *mapper = new ErrorCodesDialogMapper(m_comProtocolCode, m_model);
    connect(mapper, &ErrorCodesDialogMapper::dataChanged, this, &ErrorCodes::selectData);
...
void ErrorCodes::remove()
{
    if (!m_proxyModel->mapToSource(m_view->currentIndex()).isValid())
        return;
    ...
    if (msgBox.clickedButton() == yesButton) {
        ...
        QString sqlString = QString("DELETE FROM `KSU753ErrorCodes`"
...
void ErrorCodes::selectData()
{
    QSqlDatabase db = DBConfig::instance()->configDatabase();
    QString sqlString = QString("SELECT * FROM `KSU753ErrorCodes`"
