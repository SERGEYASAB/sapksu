...
ModbusCrc *ModbusCrc::getInstance()
{
    static ModbusCrc singletone;
    return &singletone;
}

QByteArray ModbusCrc::calcCrc(const QByteArray &data, const quint16 &offset, const quint16 &length)
{
    crc[0] = char(0xFF);
    crc[1] = char(0xFF);
    quint8 uIndex;

    for (quint16 current = offset, last = offset + length; current < last; current++) {
        uIndex = crc[0] ^ data[current];
        crc[0] = char(crc[1] ^ hi[uIndex]);
        crc[1] = char(lo[uIndex]);
    }

    return crc;
}

bool ModbusCrc::exactMatch(const QByteArray &data)
{
    if (data.size() < 2)
        return false;
...
ModbusCrc::ModbusCrc()
{
    hi = QByteArray::fromHex("00 C1 81 40 01 C0 80 41 01 C0"
    ...
    lo = QByteArray::fromHex("00 C0 C1 01 C3 03 02 C2 C6 06"
