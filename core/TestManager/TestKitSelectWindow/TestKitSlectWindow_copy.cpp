#include "TestKitSlectWindow_copy.h"
...
TestKitSlectWindow_copy::TestKitSlectWindow_copy(QWidget *parent) : QMainWindow(parent)
{
    // инициализируем базу
    m_dataBase = DBConfig::instance()->configDatabase();

    m_runTestKit = new QPushButton(tr("Выполнить"));
    connect(m_runTestKit, &QPushButton::clicked, this, &TestKitSlectWindow_copy::runTestKit);
...
void TestKitSlectWindow_copy::runTestKit()
{
    qDebug()<< "Запуск ExecutorWindow";
    ...
    ExecutorWindow_copy *testExecutor = new ExecutorWindow_copy(testKit);
    ...
    testExecutor->setVisible(false);

    QObject::connect(MainEngine::instance()->engine().rootObjects().first()->findChild<QObject*>("startWindow"), SIGNAL(playThisTestClicked()), testExecutor, SLOT(runCurrentTest()));
//11.48.03
}

QList<int> TestKitSlectWindow_copy::getTestIdListOfKit(int testKitId)
{
